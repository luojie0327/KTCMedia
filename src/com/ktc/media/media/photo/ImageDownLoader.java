...
public class ImageDownLoader {
    ...
    public ThumbnailThreadPool getThreadPool() {
    ...
    @SuppressLint("all")
    public void downloadImage(final int position, final String url, final onImageLoaderListener listener) {
        ...
        getThreadPool().execute(new Runnable() {
            @Override
            public void run() {
                Drawable drawable = getDrawableFormUrl(url);
    ...
    private Drawable getDrawableFormUrl(String url) {
        Log.i(TAG, "getDrawableFormUrl:" + url);
        int width = DestinyUtil.dp2px(mContext, 189.3f);
        int height = DestinyUtil.dp2px(mContext, 107);
        Bitmap bitmap = null;
        Bitmap bitmapResult = null;
        mtb = new MediaThumbnail();
        File file = new File(url);
        bitmap = decodeSampledBitmapFromFilePath(file.getAbsolutePath(), width, height);
        bitmapResult = ThumbnailUtils.extractThumbnail(bitmap, width, height);
    ...
    public synchronized void cancelTask(boolean isShutDown) {
        Log.i(TAG, "cancelTask: isShutDown = " + isShutDown);
