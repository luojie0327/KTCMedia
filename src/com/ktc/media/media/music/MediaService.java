...
public class MediaService extends Service {
    ...
    public int openPlayer(String path) {
        ...
        try {
            ...
            mServiceId++;
            Log.d(TAG, "player.setDataSource(), service id = " + mServiceId + ", path = " + path);
            player.prepareAsync();
    ...
    public void stopById(final int serviceId) {
        if (serviceId == mServiceId) {
            stop();
        } else {
            Log.d(TAG, "stop req ignored! client id = " + serviceId + ", current id = " + mServiceId);
        }
    }
    ...
    public void seekTo(int position) {
        ...
        if (isInPlaybackState() && position >= 0 && position <= getDuration()) {
            try {
                mSeekWhenPrepared = 0;
                Log.d(TAG, "player.seekTo(), pos = " + position);
    ...
    private class ServiceStub extends IMediaService.Stub {
        ...
        public void stopById(final int serviceId) {
            mService.get().stopById(serviceId);
        }
        ...
        public int initPlayer(String path) {
            Log.d(TAG, "initPlayer, current thread id : "
                    + Thread.currentThread().getId());
            Log.i(TAG, "musicPath:" + path);
            return mService.get().openPlayer(path);
        }
