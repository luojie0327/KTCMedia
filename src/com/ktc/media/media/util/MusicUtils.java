...
public class MusicUtils {
    ...
    public static Bitmap parserMp3(String path) {
        Mp3Id3V2Info info2 = null;
        Mp3Id3V2Info info22 = null;
        Bitmap bmp = null;
        try {
            RandomAccessFile ran = new RandomAccessFile(new File(path), "r");
            try {
                byte[] buffer = new byte[128];
                ran.seek(ran.length() - 128);
                ran.read(buffer);
                info2 = new Mp3Id3V2Info(ran);
    ...
    static class Mp3Id3V2Info {
        ...
        private int totalTagSize(byte[] b) {
            return ((((b[0] & 127) * 2097152) + ((b[1] & 127) * 16384)) + ((b[2] &
                    127) * 128)) + (b[3] & 127);
        }
        ...
        Mp3Id3V2Info(RandomAccessFile rdf) throws IOException {
            if (new String(read(rdf, 0, 3)).equals("ID3")) {
        ...
        public void removeTag() {
            for (int i = 0; this.pictureBuf[i] == (byte) 0; i++) {
            }
        }
